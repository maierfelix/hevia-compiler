...
export function parseMemberExpression(base) {
  ...
  let node = this.createNode(Type.MemberExpression);
  ...
  if (node.isComputed && this.eat(TT.COMMA)) {
    ...
    let tmp = this.createNode(Type.ArrayExpression);
    tmp.argument = args;
    node.property = tmp;
  }
...
export function parseCallExpression(base) {
  ...
  let node = this.createNode(Type.CallExpression);
...
export function parseTernaryExpression(base) {
  ...
  let node = this.createNode(Type.TernaryExpression);
